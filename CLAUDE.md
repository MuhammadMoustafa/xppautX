...
The classic page (`web/`) is legacy, frozen until T18 removes it, served
at `/v1/` (web2 is the default page, at `/`; a `/v2/` bookmark redirects
to `/`). Its check, `tools/webtest.mjs`, is not a routine gate any more
(run it only for a fix to `web/`). From Git Bash on Windows, where Node
...
The front end (`web2/`, the default page at `/`; design and plan in
...
- `xppautX` (`make xppautx`) is one program: `core/xppautx_main.c` picks
  ...
  (served at `/`) and `web/` (the legacy page, served at `/v1/`; `/v2/`
  redirects to `/`, docs/ui-v2.md T17).
