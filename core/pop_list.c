...
/* ---- edit box: a list of long editable strings (was edit_rhs.c) ---------- */

#include "edit_rhs.h"
#define EBOX_EV_MASK (ButtonPressMask|KeyPressMask|ExposureMask|StructureNotifyMask)
#define EBOX_BUT_MASK (ButtonPressMask|KeyPressMask|ExposureMask|StructureNotifyMask|\
...
int x11_do_edit_box(n,title,names,values)
int n;
char **names,**values,*title;
{
 ...
 XSelectInput(display,sb.cancel,EBOX_BUT_MASK);
 XSelectInput(display,sb.ok,EBOX_BUT_MASK);
 XSelectInput(display,sb.reset,EBOX_BUT_MASK);
 pos=strlen(sb.value[0]);
 colm=(pos+strlen(sb.name[0]))*DCURX;
 ...
 XSelectInput(display,sb.cancel,EBOX_EV_MASK);
 XSelectInput(display,sb.ok,EBOX_EV_MASK);
 XSelectInput(display,sb.reset,EBOX_EV_MASK);
