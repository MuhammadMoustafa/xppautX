...
static int ask_base(const char *title, char *buf, size_t n)
{
    ...
    if (!file_selector((char *)title, buf, (char *)"*.set")) return 0;
    dot = strrchr(buf, '.');
...
int xpp_session_save(const char *base)
{
    ...
    if (NBifs > 1) { /* a diagram exists (save_diagram's own empty check) */
        ...
        if (fp == NULL) {
            err_msg((char *)"Cannot open AUTO file");
            return 0;
        }
        if (save_auto_file(fp) != 1) {
            fclose(fp);
            err_msg((char *)"Empty diagram -- nothing to save");
            return 0;
        }
        fclose(fp);
