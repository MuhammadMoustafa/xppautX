...
| W22 | #35 | Sanitizers on macOS: a macos-sanitizers CI job building xppautX with Apple clang's AddressSanitizer and UBSan (no LeakSanitizer: not on Apple Silicon) and running tools/asancheck.sh's checks under it (smoke run, every example, unit tests, servercheck, autocheck), asancheck.sh taught a no-leaks mode for it; every report fixed | none | ready |
| W23 | #36 | Clang on Windows: a windows-clang CI job with MSYS2's CLANG64 toolchain (clang + libc++ + compiler-rt), first building xppautX and passing the windows-core checks (a third compiler on Windows), then an ASan + UBSan build (compiler-rt supports both on x86_64 MinGW) running asancheck.sh's checks; every report fixed; the local MinGW gcc stays the default build | none | ready |
