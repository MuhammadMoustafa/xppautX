...
| W13d | #25 | The macOS window on: the macOS builds (CI's artifact and the release's archive) with `WINDOW=1` once CI's `WINDOW=1` step builds, and a tester's how-to (where to download, opening an unsigned binary: `xattr -dr com.apple.quarantine`, what to report); the tester's run is #4 | CI's macOS `WINDOW=1` build passing (first seen on the push of 2026-09-24) | blocked |
| W13e | #25 | The Linux window in the release without a hard dependency: the window's GTK/WebKitGTK part built as a library beside xppautX and loaded only in window mode; when WebKitGTK is missing xppautX says so with the install command for the system (apt, dnf, pacman, zypper by /etc/os-release) and opens the browser; `-silent`, `--server` and `--browser` never load it; the release's Linux job builds and ships it | none | ready |
