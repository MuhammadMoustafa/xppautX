...
| W18 | #31 | Windows passes the checks it runs since W17: `xppautX.exe --server` exits at end of input from NUL (autocheck timed out), the 4 web2check failures on the runner (a field's blur commit, the focus back to the Values/Text buttons, AUTO's Stop status), every check step of every job runs even after an earlier check failed (the examples and their artifact included; the build still gates them); a macos-browser job (web2check on macOS, which W17's card asked for and missed); then tests/examples.windows.md5 from CI | none | running |
