...
| W14 | #26 | Smaller downloads, a faster and fairer CI: the release archives and CI artifacts carry stripped binaries (Linux 15.2 -> 3.3 MB, Windows 18.5 -> 4.1 MB; debug info kept in local builds); the Windows job's unit tests in parallel; the macOS job also runs the checks that do not compare Linux's exact checksums (modecheck, autocheck's tolerant parts, every example for crashes) | none | ready |
