...
| W13f | #25 | The window fits the screen: its first size (1280x840, scaled by the display's DPI) shrunk to the work area (the screen less the taskbar) of the monitor it opens on, and centred there (Windows; GTK on Linux; macOS left to W13d) | W13e (the same file) | running |
