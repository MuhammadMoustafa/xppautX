...
| T32 (running) | The rest of T31: the core's forms that still send every field as text (Range and Range over equilibria, nullclines, the array plot, Graphic stuff/grobs, Numerics, Viewaxes/Window, Initial data range, Transpose, Fit, Stochastic, ...) send each field's kind from their call sites, so every box in the program is checked by its kind | T31 | yes (field kinds) | servercheck: each annotated form's fields carry their kinds |
