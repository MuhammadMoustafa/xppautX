...
| T29 (running) | From hands-on testing: AUTO's labelled points each with a shape of their type instead of one × for all: HB filled circle, LP triangle, BP diamond, PD square, TR star, UZ inverted triangle, MX a bold ×, EP a small bar; the plain numbered points (Npr's regular output labels) a small tick, so a long run is not a row of crosses; the key shows each shape beside its code and name (A7: shape and text, not colour alone) | none | no | web2 unit test of type -> shape; web2check auto: the key's shapes match the drawn ones |
