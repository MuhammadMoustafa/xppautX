...
| T30 (ready) | From hands-on testing: Fit always at hand, on both plots. The user uses it all the time to find the picture again after a scroll or zoom in a wrong place: a Fit button in the plot's own corner (over the chart, always visible, with an icon and the word), on the main plots (Window/Fit, as the toolbar's Fit today) and on the AUTO diagram (which has none: fit its view to all its points, client-side); the toolbar's Fit stays | none | no | web2check: the corner Fit is visible on a plot and on the diagram; after a zoom far off, it brings every curve/point into view |
