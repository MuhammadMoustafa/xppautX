...
| T22 (in progress) | AUTO settings as data: the core sends AUTO's Numerics, Parameter choice, axes and Mark values (user points) as a data event and takes a set command for them, so web2 edits them at any time (during a run too: marked pending, applied when idle) instead of through the core's forms; only Run, Grab, Clear and File wait for idle | T21 | yes | servercheck: the event matches the forms' values, a set changes the next run; web2check: edit Numerics during a run, applied at idle, the run after uses it |
| T23 | AUTO in plain words: a stop reason code set where autlib1.c/autlib2 end a branch (Par Min/Max, Norm Min/Max, Npts reached, Stop, a stopping Mark value, no convergence at Dsmin: fort.9's NOTE) and sent to the page, shown in the status strip ("Stopped: parameter reached Par Max (20)"); label types spelled out on hover (EP End point, MX No convergence, LP Fold, HB Hopf, BP Branch point, PD Period doubling, TR Torus, UZ Marked value); every Numerics field named plainly with AUTO's short name kept (e.g. "Max points (NMX)"), a tooltip (what it does, valid values, what each option means), integer or float validation and ranges (Dsmin <= |Ds| <= Dsmax), also checked in the core; docs/manual/09-auto.md updated | T22 | yes | servercheck: each stop reason on a model built to hit it (Par Max, Norm Max, Npts, Stop, MX); saved .b/.d and verify.sh unchanged; web2check: status names the reason, a float in an integer field is refused with a message |
